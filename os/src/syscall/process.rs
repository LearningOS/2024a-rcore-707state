...
use crate::{
    ...
    syscall::{SYSCALL_EXIT, SYSCALL_GET_TIME, SYSCALL_YIELD},
...
pub fn sys_exit(exit_code: i32) -> ! {
    ...
    current_task.syscall_times[SYSCALL_EXIT] += 1;
...
pub fn sys_yield() -> isize {
    ...
    let inner = task_manager.inner.exclusive_access();
    let mut current_task = inner.tasks[inner.current_task];
    current_task.syscall_times[SYSCALL_YIELD] += 1;
...
pub fn sys_get_time(ts: *mut TimeVal, _tz: usize) -> isize {
    ...
    let inner = task_manager.inner.exclusive_access();
    let mut current_task = inner.tasks[inner.current_task];
    current_task.syscall_times[SYSCALL_GET_TIME] += 1;
    let us = get_time_us();
    unsafe {
        *ts = TimeVal {
            sec: us / 1_000_000,
